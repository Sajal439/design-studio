...
const navLinks = [
  {
    href: "/",
    label: "Home",
    description: "See our latest material-first homepage.",
    icon: SwatchBook,
  },
  ...
  { href: "/estimator", label: "Estimator" },
  {
    href: "/consultation",
    label: "Consultation",
    description: "Book a free call or showroom discussion.",
    icon: MessageCircle,
  },
];
    ...
    <header className="sticky top-0 z-50 w-full border-b bg-background/80 backdrop-blur-sm">
      <div className="container mx-auto flex h-16 items-center justify-between px-4">
        ...
        <nav className="hidden md:flex items-center gap-6">
          {navLinks
            .filter(
              (link) => link.href !== "/consultation" && link.href !== "/",
            )
            .map((link) => (
              <Link
                key={link.href}
        ...
        <Sheet>
          <SheetTrigger asChild className="md:hidden">
            <Button
              variant="outline"
              size="icon"
              className="rounded-full border-slate-200 bg-white/80 shadow-sm"
            >
          ...
          <SheetContent
            side="right"
            className="w-[88vw] max-w-sm border-l border-slate-200 bg-gradient-to-b from-white via-slate-50 to-slate-100 p-0"
            ...
            <div className="flex h-full flex-col">
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-3 max-w-xs text-sm leading-6 text-slate-600">
                      Browse designs, estimate material costs, and connect with
                      the showroom team in one place.
                    </p>
                  </div>
                  <SheetClose asChild>
                    <Button
                      variant="ghost"
                      size="icon-sm"
                      className="rounded-full border border-slate-200 bg-white text-slate-600"
                    >
              ...
              <nav className="flex-1 space-y-3 overflow-y-auto p-5">
                {navLinks.map((link) => {
                  const Icon =
                    link.icon ??
                    (link.href === "/estimator" ? Calculator : SwatchBook);
