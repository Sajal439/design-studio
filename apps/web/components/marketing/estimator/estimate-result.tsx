...
import { CompareGradesModal } from "./compare-grades-modal";
import type { MaterialGrade } from "@/lib/estimator/types";

const WHATSAPP_NUMBER = "7355801455";

// Known cost multipliers per grade relative to STANDARD
const GRADE_MULTIPLIERS: Record<MaterialGrade, number> = {
  BUDGET: 0.72,
  STANDARD: 1.0,
  PREMIUM: 1.42,
};

function buildGradeCompareData(result: EstimationResult) {
  const current = result.grade as MaterialGrade;
  const currentMult = GRADE_MULTIPLIERS[current];
  return (Object.keys(GRADE_MULTIPLIERS) as MaterialGrade[]).map((grade) => {
    const ratio = GRADE_MULTIPLIERS[grade] / currentMult;
    return {
      grade,
      costMin: Math.round(result.summary.totalCostMin * ratio),
      costMax: Math.round(result.summary.totalCostMax * ratio),
    };
  });
}
...
interface EstimateResultProps {
  ...
  /** Called when user picks a different grade in the compare modal */
...
export function EstimateResultDisplay({ result, onChangeGrade }: EstimateResultProps) {
  const [copied, setCopied] = useState(false);
  const [showCompare, setShowCompare] = useState(false);
  const gradeData = buildGradeCompareData(result);
  ...
  return (
    <div className="space-y-8 print-root">
      ...
      {showCompare && (
        <CompareGradesModal
          grades={gradeData}
          currentGrade={result.grade as MaterialGrade}
          onSelectGrade={(g) => {
